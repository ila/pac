...
#if PAC_DEBUG
	PAC_DEBUG_PRINT("GetTableMetadata: looking up '" + table_name + "' (normalized: '" + normalized_name + "')");
	PAC_DEBUG_PRINT("GetTableMetadata: metadata store has " + std::to_string(table_metadata.size()) + " entries");
	for (const auto &entry : table_metadata) {
		PAC_DEBUG_PRINT("  stored table: '" + entry.first + "'");
	}
#endif
	if (it != table_metadata.end()) {
#if PAC_DEBUG
		PAC_DEBUG_PRINT("GetTableMetadata: FOUND metadata for '" + normalized_name + "'");
		PAC_DEBUG_PRINT("  - links count: " + std::to_string(it->second.links.size()));
		PAC_DEBUG_PRINT("  - protected_columns count: " + std::to_string(it->second.protected_columns.size()));
		PAC_DEBUG_PRINT("  - primary_key_columns count: " + std::to_string(it->second.primary_key_columns.size()));
		for (const auto &link : it->second.links) {
			PAC_DEBUG_PRINT("  - link to: " + link.referenced_table);
		}
#endif
		return &it->second;
	}
#if PAC_DEBUG
	PAC_DEBUG_PRINT("GetTableMetadata: NOT FOUND metadata for '" + normalized_name + "'");
#endif
	return nullptr;
}
